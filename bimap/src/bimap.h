...
namespace intrusive {
    ...
    struct map_iterator {
        ...
        // Element which is currently referenced by iterator.
        // Dereferencing end_left() is undefined.
        // Dereferencing invalid iterator is undefined.
        ...
        // Going to the next left.
        // Increment of end_left() is undefined.
        // Increment of invalid iterator is undefined.
        iterator &operator++() {
        ...
        // Going to the previous left.
        // Decrement of begin_left() is undefined.
        // Decrement of invalid iterator is undefined.
        iterator &operator--() {
        ...
        // left_iterator is referencing to the left element of some pair.
        // This function returns iterator of the right element of the same pair.
        // end_left().flip() returns end_right().
        // end_right().flip() returns end_left().
        // flip() using for invalid iterator is undefined.
    ...
    private:
        ...
        // Invariatn: nodes have unique keys
    ...
    private:
        ...
        template<typename Left, typename Right, typename CompareLeft, typename CompareRight>
        friend bool operator==(::bimap<Left, Right, CompareLeft, CompareRight> const &a,
...
struct bimap {
    ...
    // Inserting pair (left, right) returns left iterator.
    // If left or right is already in bimap, there is no insertion and returns end_left().
    ...
    // Removes an element and its corresponding paired.
    // erase of invalid iterator is undefined.
    ...
    // similar to erase but for key.
    // Returns whether the pair was deleted or not.
    ...
    // Returns element's iterator or end() if the element wasn't found.
    ...
    // Returns an opposite element by element.
    // If there is no element - throws std::out_of_range.
    ...
    // Returns an opposite element by element.
    // If there is no element, adds it to bimap and puts default element on the opposite side and returns reference to it.
    // If the default element is already in the opposite pair -
    // have to change the corresponding element to the required one (watch tests).
    ...
    // lower and upper bounds for each side.
    // Returns iterator for corresponding elements.
    ...
    // Returns iterator for the min left.
    left_iterator begin_left() const {
        return map_left.begin();
    }

    // Returns iterator for the first left after the max left.
    left_iterator end_left() const {
        return map_left.end();
    }

    // Returns iterator for the min right.
    right_iterator begin_right() const {
        return map_right.begin();
    }

    // Returns iterator for the first right after the max right.
