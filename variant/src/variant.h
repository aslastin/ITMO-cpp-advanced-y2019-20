...
namespace variant_helpers {
    // casts base to variant with correct qualifiers
    ...
    // Helpful functions for variant constructing
...
namespace variant_implementation {
    // Wrapper for type with getters for Union_storage. Always has trivial destructor.
    ...
    // The lowest variant base - trivial_dtor_base
    ...
    struct Trivial_dtor_base {
        ...
        constexpr Trivial_dtor_base(in_place_index_t<I>, Args &&... args)
                : u(in_place_index<I>, std::forward<Args>(args)...), current_index{I} {}
    ...
    struct Copy_assign_base<false, Ts...> : Variant_move_ctor_base<Ts...> {
        ...
        constexpr Copy_assign_base &
        operator=(Copy_assign_base const &other) noexcept(Traits<Ts...>::NOTHROW_COPY_ASSIGN) {
...
template<size_t I, typename... Ts>
constexpr std::add_pointer_t<variant_alternative_t<I, variant<Ts...>> const>
get_if(variant<Ts...> const *ptr) noexcept {
    static_assert(I < sizeof...(Ts), "index must be less than amount of alternatives");
    if (ptr != nullptr && ptr->index() == I) {
        return std::addressof(variant_helpers::get<I>(*ptr));
    }
