...
/*
Содержит всякие ништячки такие как:
    in_place_index,
    in_place_type,
    variant_size,
    variant_alternative,
    variant_npos,
    instance_by_index - тип по индексу
    index_by_instance - индекс по типу
    Traits - полезная свойства для построения variant
    appropriate_index - подходящий индекс для converting constructor
    is_unique_type - проверяет уникальность заданного типа в наборе типов
*/
...
/// Содержит разные константы для проверки свойств variant
template <typename... Ts>
struct Traits {
...
/// Appropriate_index
/// В общем, здесь будем сверху вниз конструировать промежуточные классы с помошью которых
/// по итогу полностью проверим тип на соответствие условию конвертирующего конструктора
